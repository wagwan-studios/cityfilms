<footer>
    <div class="content-wrapper">
        <h2>Let's Connect</h2>
        <div class="footer-columns">
            <span class="bold-text border-top border-bottom border-right largespan order-1"> <a href="mailto:Sayhello@cityfilms.com">Sayhello@cityfilms.com.pk</a> </span>
            <span class="bold-text border-top border-bottom normalspan text-center order-2"> <a href="tel:03336121121">03336121121</a> </span>
            <span class="bold-text border-bottom border-right largespan order-3"> <a href="#">House # Dummy , Karachi Pakistan</a> </span>
            <span class="border-bottom border-right smallspan text-center order-4"> <a href="#"><i class="ri-instagram-line"></i></a> </span>
            <span class="border-bottom smallspan text-center order-5"><a href="#"><i class="ri-vimeo-line"></i></a></span>

        </div>
        <div class="logo-wrapper">
            <div class="logo-description">
                <img src="{{ asset('assets/images/CITYFILMS.png') }}" alt="logo">
                <p>A full-service video production company based in San Diego, CA.
                    Our work is built around the passion of telling stories through visuals. By putting
                    collaboration at the center of our process</p>
            </div>
            <div class="newsletter">
                <form class="newsletter-form">
                    <label for="email" class="newsletter-label">Subscribe to our newsletter</label></br>
                    <input type="text" placeholder="email address" id="email" name="email">
                    <button disabled type="submit" id="submit"> <i class="ri-arrow-right-line"></i> </button>
                </form>
            </div>
        </div>
    </div>
</footer>
<div class="link-dynamic-cursor"></div>
